...
Java_com_ymlion_mediasample_util_YuvUtil_convertToARGB(JNIEnv *env, jclass type,
    ...
    if (orientation > 0) {
        ...
        ARGBRotate((const uint8_t *) rgbData, src_stride, (uint8_t *) rotateData, dst_stride, width,
                   height, orientation);
    } else {
        rotateData = rgbData;
    }
    ...
    if (ANativeWindow_lock(nativeWindow, &windowBuffer, NULL) < 0) {
        ...
        ARGBScale((uint8_t *) rotateData, src_stride, w, h, dst, windowBuffer.stride * 4, dstWidth,
                  dstHeight, scaleMode);
